...
//
// NAVE Task Dashboard UI (Batch 8D).
...
frappe.pages["nave-task-dashboard"].on_page_load = function (wrapper) {
	...
	page.add_inner_button(__("← Nave Home"), () => {
		frappe.set_route("nave-home");
	});
	page.add_inner_button(__("NAVE Tasks"), () => {
	...
	const add_styles = () => {
		...
		$(`<style id="nave-task-dashboard-style">
			...
			.ntd-kpi-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(140px,1fr)); gap:10px; margin-bottom:14px; }
			.ntd-kpi { background:#fff; border-radius:12px; padding:14px; border-top:4px solid #1683d8; box-shadow:0 2px 10px rgba(23,59,103,.06); }
			.ntd-kpi.success { border-top-color:#16a36a; }
			.ntd-kpi.warning { border-top-color:#f59e0b; }
			.ntd-kpi.alert { border-top-color:#e34b4b; }
			.ntd-kpi-label { color:#64748b; font-size:12px; font-weight:600; }
			.ntd-kpi-value { color:#173b67; font-size:24px; font-weight:700; margin-top:6px; }
			.ntd-widget-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(280px,1fr)); gap:12px; margin-bottom:14px; }
			.ntd-panel { background:#fff; border-radius:12px; padding:14px; box-shadow:0 2px 10px rgba(23,59,103,.06); min-height:120px; }
			...
			.ntd-chart-grid { display:grid; grid-template-columns:repeat(auto-fit,minmax(320px,1fr)); gap:12px; }
			.ntd-chart-box { min-height:260px; }
	...
	const shell_html = () => `
		<div class="ntd-wrap">
			...
			<div class="ntd-chart-grid">
				${CHARTS.map(
					(c) => `
					<div class="ntd-panel" data-chart="${escape(c.type)}">
	...
	const make_link_control = (parent_sel, fieldname, options, label) => {
		const $parent = page.main.find(parent_sel);
		$parent.empty();
	...
	const read_filters = () => {
		const filters = {};
		page.main.find(".ntd-filter").each(function () {
	...
	const write_filters = (filters) => {
		...
		page.main.find('.ntd-filter[data-key="from_date"]').val(values.from_date || "");
		page.main.find('.ntd-filter[data-key="to_date"]').val(values.to_date || "");
		page.main.find('.ntd-filter[data-key="priority"]').val(values.priority || "");
		page.main.find('.ntd-filter[data-key="status"]').val(values.status || "");
		["assigned_to", "department", "project"].forEach((key) => {
	...
	const set_loading = (loading) => {
		...
		page.main.find(".ntd-refresh, .ntd-apply, .ntd-clear").prop("disabled", !!loading);
	};

	const show_filter_error = (message) => {
		const $err = page.main.find(".ntd-filter-error");
		if (message) {
	...
	const destroy_chart = (chart_type) => {
		...
		page.main.find(`#ntd-chart-${chart_type}`).empty();
	};
	...
	const render_widget = (widget_type, payload, error) => {
		const $body = page.main.find(`[data-widget="${widget_type}"] .ntd-widget-body`);
		if (error) {
	...
	const render_chart = (spec, payload, error) => {
		const $box = page.main.find(`#ntd-chart-${spec.type}`);
		const $note = page.main.find(`[data-chart="${spec.type}"] .ntd-chart-note`);
	...
	const load_metadata = async () => {
		...
		state.metadata = meta || {};
		fill_select(page.main.find('.ntd-filter[data-key="priority"]'), meta.priorities || []);
		fill_select(page.main.find('.ntd-filter[data-key="status"]'), meta.statuses || []);
		return meta;
	};
	...
	const load_dashboard = async () => {
		...
		set_loading(true);
		page.main.find(".ntd-global-error").hide().text("");
		page.main.find(".ntd-kpi-grid").html(`<div class="ntd-loading">${__("Loading KPIs...")}</div>`);
		WIDGETS.forEach((w) => {
			page.main
		...
		try {
			...
			page.main.find(".ntd-global-error").text(message).show();
		} finally {
			if (request_id === state.request_id) {
				set_loading(false);
			}
	...
	const bind_events = () => {
		page.main.find(".ntd-apply").on("click", () => load_dashboard());
		page.main.find(".ntd-refresh").on("click", () => load_dashboard());
		page.main.find(".ntd-clear").on("click", () => {
	...
	const boot = async () => {
		...
		try {
			...
			page.main.find(".ntd-global-error")
